...
Tauri app. Plus test-fixture cleanup (fictitious identifiers).

### Rust crate (`sol-p4-tools`)

- **`sync` module** — `p4_sync(paths: &[String]) -> Result<P4SyncRawOutput, String>`
  returning raw stdout / stderr. Output-line classification (updated /
  already_current / errors) is intentionally left to the caller, as UI
  representation varies by tool. Empty input returns default immediately.
- **`self_update` module** — Windows binary self-replacement using the
  rename-running-exe + `p4 sync` trick. Three entrypoints:
  - `get_current_binary_path() -> Result<PathBuf, String>`
  - `cleanup_stale_binary() -> Result<bool, String>` — start-of-app cleanup
    of any leftover `<exe>.old` from the previous update cycle.
  - `p4_update_and_restart(app: &AppHandle) -> Result<UpdateResult, String>`
    — performs the full cycle: rename → sync → spawn detached → graceful exit
    via `app.exit(0)`. Reverts the rename on failure. Deploy-path gating
    (production vs. dev) is left to the caller — this crate is a plain wrapper.
- Test fixtures (`partition.rs`, `p4.rs`) switched to fictitious identifiers
  (`user = "alice"`, `stream = "release"`) for public-repo hygiene.
...
으로 항상 UTF-8 bytes emit 하도록 보장.

### 배경

P4 server 가 Unicode mode 일 때 사용자 env default `P4CHARSET=auto` 가
client console codepage 따라 charset 자동 결정 — Tauri pipe stdio 환경에서
잘못 결정되어 stdout 이 CP949 또는 double-encoded garbage byte stream →
Rust 의 `from_utf8_lossy` / `MultiByteToWideChar` 어떤 decode 도 정확하지
않음. `P4CHARSET=utf8` 명시 시 P4 client 가 그 charset 정확히 사용 (UTF-8
byte sequence — `EA B8 B0` = 기, `ED 9A 8D` = 획 등 — Windows hex dump 로
확인).
...
Windows release 빌드 (`chcp 949`) 에서 v0.3.4 적용 후 깨진 패턴이
`占썼영占쏙옙[占쏙옙획]` 형태로 변화 — UTF-8 bytes 가 CP_ACP (CP949) 로
잘못 decode 된 전형 (UTF-8 `기` = `\xEA\xB8\xB0` 의 첫 2 byte `\xEA\xB8`
가 CP949 에서 `占` 한자에 매핑되는 특징).

→ Windows P4 client 가 pipe stdio (Rust Command 의 stdout) 에서는 UTF-8
emit. console output 만 codepage (CP949) 변환 적용. v0.3.4 의 CP_ACP only
decode 는 잘못된 가정.
...
`chcp 65001` 후 정상 = P4 client 가 stdout 에 console output codepage 적용.
Rust 의 `from_utf8_lossy` 가 CP949 bytes 를 UTF-8 로 해석 시 깨짐. P4 server
가 non-unicode 모드라 `-C utf8` / `P4COMMANDCHARSET` 등도 효과 X.
...
- `parse_p4_changes` — single / multi / empty / malformed line skipped /
  **korean description preserved** (mojibake 회귀 방지) 5 케이스.

### Breaking 가능성

- `P4Change.time` 타입 변경 (string → i64) — caller TS / Rust 코드 영향.
  Date 변환은 frontend 책임. caller 측 UI (e.g. pending-changes panel) 도
  동시 업데이트 필요.
...
기존 `check_stale_revisions` 가 stale 한 file 목록을 반환 — UI 가 수백 file
나열 같은 형태로 표시. 사용자 멘탈 모델은 CL 단위 ("누가 어떤 CL 을 sync
안 했는가") 라 file 목록은 정보 압축이 부족.

### Rust crate (`sol-p4-tools`)

- `P4Change` struct (`#[serde(rename_all = "camelCase")]`) — `number / user /
  user_fullname / client / time / description`. `user_fullname` 은 `p4 users`
  로 lookup 한 사람 이름. lookup 실패 시 user id 와 동일.
...
silent 반환했음. 호출 측의 store 가 "0건 검출" 로 오인해 "외부 변경" 표시
가 사라지는 회귀로 이어질 수 있어 정정.
...
전제) — 이제 throw 됨. 호출 측이 try/catch wrap 또는 toast 등으로 surface
하도록 업데이트 필요.

## v0.3.0 (2026-04-28)

P4 protections + delete/revert helpers. Backported from a downstream tool — supports a read-only gating feature based on P4 folder permissions.
...
- `p4::p4_delete(path)` — mark file for delete in the pending CL. Used by apps that surface a "delete this file" action under Perforce control (e.g., file deletion in a designer UI).
...
Windows UX + branch-detection robustness, environment diagnostics. Backported from a downstream tool.
...
Initial scaffolding — extracted from downstream tools.
